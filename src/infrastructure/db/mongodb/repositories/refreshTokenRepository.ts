...
export class RefreshTokenRepository implements IRefreshTokenRepository {
  ...
  async remove(token: string): Promise<boolean> {
    const removedRefreshToken = await refreshTokenModel.findOneAndDelete({
      token,
    });
  ...
  async updateOne(token: string, newToken: string , expireIn: Date): Promise<boolean> {
    const update = await refreshTokenModel.findOneAndUpdate(
      { token },
      { token: newToken , expireIn }
    );
